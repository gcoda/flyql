...
const builder = createCompiler({
  ...
  _if_: triar((opt, yes = true, no = false) => {
    return opt ? yes : no
  }),
  _if: compiler => value => props => {
    const isArray = Array.isArray(value)
    const conditionExpression = isArray ? value[0] : value
    const condition = compiler(conditionExpression)(props)
    if (!!condition) {
      return isArray && value[1] ? compiler(value[1])(props) : true
    } else {
      return isArray && value[2] ? compiler(value[2])(props) : false
    }
  },
  ...
  _concat: compiler => value => props => {
    ...
    return pick(compiler, value, props) || ''
  },
